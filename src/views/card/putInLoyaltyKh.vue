								...
								<div class="topsercah">
									<el-form :inline="true" class="el-form-class" :model="listQuery" >
										<el-form-item label="查询" >
												...
												<el-date-picker
												v-model="listQuery.selLong" type="date" placeholder="选择日期" value-format="timestamp" style="width: 145px;">
												</el-date-picker>
										</el-form-item>
										<el-form-item label="至今,">
											  <el-select v-model="listQuery.YMsel" placeholder="请年或月" style="width: 65px;">
													<el-option
										...
										<el-form-item label="平均订单金额满">
											<el-input v-model="listQuery.avgMoney" placeholder="平均订单金额满" style="width: 80px;" ></el-input>
										</el-form-item>
	...
	export default {
		...
		data() {
			return {
				...
				listLoading: true,
				listQuery: {
					selLong:new Date().getTime() - 3600*1000*24*65,
					YMsel:'月',
					avgMoney:400,
					pageSize:20,
					pageNum:1
...
/* 		created() {
        ...
        return Y+M+D;
			...
			timestampSetStart:function(timestamp){
				var date = new Date(timestamp);
				date.setHours(0);
				date.setMinutes(0);
				date.setSeconds(0);
			...
			init() {
				...
				this.timestampSetStart(this.listQuery.selLong);
				card.log("/pageByOrders",this.listQuery).then(res => {
			...
			onSubmit() {
				...
				if(list == null  || list.length == undefined || list.length == 0)
				{
					...
					this.form.log[0].condition=this.timestampToTime(this.listQuery.selLong)+" 前注册，且 "+this.listQuery.YMsel+" 平均金额满 "+this.listQuery.avgMoney+" 元的用户。"
				}
