					...
					<el-row>
						<div class="topsercah">
							<el-form :inline="true" class="el-form-class" :model="listQuery" >
								<el-form-item label="查询近" >
										<el-input v-model="listQuery.monthNum" placeholder="月份" style="width: 80px;" ></el-input>
								</el-form-item>
								<el-form-item label="个月未下单的用户">
								</el-form-item>
			...
			return {
				...
				form: {
					log:[{
						...
						guid: "",
						putType: "激活可能流失的用户",
						condition: "根据近期某多少个月前未下单的用户进行投放。",
						summary: "",
						cardId:0,
						stockCnt:0,
						collarCnt:0,
		...
		methods: {
			...
			{ 
			   ...
			   if(monthNums > 0){
				   ...
				   year = year - (1 + parseInt((-monthNums)/12));
				   month =12 - (-monthNums)%12;
				   clock = year + "-";
				   if(month < 10)
				   clock += "0";
				   clock += month + "-"; 
			...
			init() {
				...
				var timeDate = this.getTimeDate(this.listQuery.monthNum);
				
					this.listQuery.timeLong = this.longTimestamp(timeDate);
					//pageSize:20,
					//pageNum:1
				
				card.log("/pageByNotOrdersDate",this.listQuery).then(res => {
			...
			onSubmit() {
				...
				if(list == null  || list.length == undefined || list.length == 0)
				{
					...
					this.form.log[0].condition="查询"+this.listQuery.monthNum+" 个月未下单的客户。";
				}
