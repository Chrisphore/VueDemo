								...
								<div class="topsercah">
									<el-form :inline="true" class="el-form-class" :model="listQuery" >
										 <el-form-item label="查询" >
												...
												<el-date-picker v-model="listQuery.smallDate" type="date" placeholder="选择日期" style="width: 145px;"></el-date-picker>
										</el-form-item>
										
										<el-form-item label="至" >
												<span class="demonstration"></span>
												<el-date-picker v-model="listQuery.bigDate" type="date" placeholder="选择日期" style="width: 145px;"></el-date-picker>
										</el-form-item> 
										<el-form-item label="注册,且从未下过单的用户.">
										</el-form-item>
	...
	export default {
		...
		data() {
			return {
				...
				listLoading: true,
				listQuery: {
					//smallDate:new Date('2018-05-25'),
					smallDate:this.dateSetStart(new Date(new Date()-365*24*60*60*1000)),
		...
		methods: {
			...
			init() {
				...
				card.log("/pageByKhLogonDate",this.listQuery).then(res => {
			...
			onSubmit() {
				...
				if(list == null  || list.length == undefined || list.length == 0)
				{
					...
					this.form.log[0].condition="查询"+this.dateFormat(this.listQuery.smallDate)+" 至 "+this.dateFormat(this.listQuery.bigDate)+" 注册,且从未下过单的客户。";
				}
