<template>
	<el-dialog title="品牌类型编辑" :visible="ishow" center width="40%" @open="openinit" @close="closedialog">
		<!-- <el-row>
			<div class="topBtn">
			...
			<el-row>
				<el-col :span="20">
					<el-form-item label="图片地址:">
						...
						<tool-img ref="toolsimg" :typeid="5" :imgurl="form.imgUrl" :imgkey="imgkey" :uploadstu="true" :imgwidth="100" :imgheight="50" :islimit="false" :isdeleteimg="false">
						</tool-img>
						<el-tag>图片大小：98*50</el-tag>
			 ...
			 <el-row>
				<el-col :span="20">
					<el-form-item label="微信卡券logo地址:">
						<tool-img ref="toolsimgLogo" :typeid="5" :imgurl="form.logoUrl"  :uploadstu="true" :imgwidth="100" :imgheight="50" :islimit="false" :isdeleteimg="false">
						</tool-img>
						<el-tag>图片大小：300*300</el-tag>
						<el-tag>点击+进行上传</el-tag>
					</el-form-item>
				</el-col>
			</el-row> 
			...
			<el-row>
				<el-col :span="20">
					<el-form-item label="是否推荐:">
						<el-select v-model="form.groom" placeholder="请选择">
							<el-option
								v-for="item in groomOptions"
								:key="item.value"
								:label="item.label"
								:value="item.value">
							</el-option>
...
export default {
  ...
  methods: {
    ...
    openinit() {
      ...
      if (this.$refs.toolsimgLogo) {
        this.$refs.toolsimgLogo.cdnurl = "";
      }
      this.form = {
        id: 0,
        name: "",
      ...
      if (this.id > 0) {
        basic.brand("GETID", this.id).then(res => {
    ...
    onSubmit() {
      ...
      if (this.$refs.toolsimgLogo.cdnurl) {
        this.form.logoUrl = this.$refs.toolsimgLogo.cdnurl;
      }
      if (!this.form.imgUrl&&this.form.imgUrl.toString() == "") {
        this.$message.warning("图片必须上传");
        return;
      }
      if (!this.form.logoUrl&&this.form.logoUrl.toString()== "") {
        this.$message.warning("微信卡券logo必须上传");
        return;
	  }
	  if(this.form.status=='') {
		 this.$message.warning("请选择显示状态");
        return;  
	  }
      basic.brand(method, this.form).then(res => {
